...
# That paper compares the results of JIM function boundary detection with
# other tools. This script is used to run the "JIMA" tool and obtain data
# files for that tools function boundary detection.
#
# To run this tool, make sure "processJil" is on your search path or modify the
...
#    print('Processing {:s}'.format(baseName))
    cmd=jimaBin+' --file {:s}'.format(fileName,baseName)
...
def main():
   ...
   parser = argparse.ArgumentParser(formatter_class=formatter,
                                    description=textwrap.dedent('''\JIMA Function Boudnary Detection Results Interface (from JIMA toolkit). 

   This program runs JIMA and generates .newgt files for each 
   ...
   Make sure that the processJil executable is on your search path
   '''))
